...
- `Jina Bolton`_ [`@jina`_] is a regular speaker at web conferences,
...
- `Scott Kellum`_ [`@scottkellum`_] created `Singularity`_
  for managing asymmetric grids,
  as well as `Sassy Math`_, `Modular Scale`_, and more.
- `Rachel Nabors`_ [`@crowchick`_] is an award-winning comicker,
  developer, Susy contributor, and a regular speaker at web conferences.
- `Danny Palmer`_ [`@dannyprose`_] has helped
  create & maintain the Susy docs for the past year.
  He is also the creator of `Middleman-HTML5BP-HAML`_,
  a project template using HTML5 Boilerplate, HAML, Sprockets,
  Susy, and more.
- `Sam Richard`_ [`@snugug`_] is an organizer for various Sass & RWD
  meetups in New York City, and a co-maintainer of
  `Breakpoint`_, `Sassy Math`_, `Singularity`_, and so on.
- `Tsachi Shlidor`_ [`@shlidor`_] created `Sasson`_
  Drupal theme (among others), and the `Salsa`_ layout system.
- `Mason Wendell`_ [`@codingdesigner`_] is the creator of `Breakpoint`_
